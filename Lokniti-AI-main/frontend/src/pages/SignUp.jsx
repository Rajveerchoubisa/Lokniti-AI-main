...
//     if (strong.test(pw) && pw.length >= 10) return "Strong";
//     if ((/([A-Za-z].*\d)|(\d.*[A-Za-z])/.test(pw)) && pw.length >= 8) return "Medium";
//     return "Weak";
//   }
...
//               <p className="text-blue-200 text-sm">Join Lokniti AI — it only takes a minute</p>
//             </div>
//           </div>

//           <form onSubmit={handleSubmit} className="space-y-4" noValidate>
//             {error && <div className="rounded-md bg-red-900/60 text-red-200 px-4 py-2 text-sm">{error}</div>}
//             {successMsg && <div className="rounded-md bg-green-900/60 text-green-200 px-4 py-2 text-sm">{successMsg}</div>}

//             <div>
//               <label htmlFor="name" className="block text-sm font-medium text-blue-100">Full name</label>
//               <div className="mt-1">
//                 <input
//                   id="name"
//                   name="name"
...
//               <p className="mt-2 text-xs text-blue-300">Password strength: <span className="font-semibold text-white">{passwordStrength(password) || "—"}</span></p>
//             </div>
...
//         <p className="mt-4 text-center text-xs text-blue-400">By creating an account, you agree to Lokniti AI's <a href="/terms" className="underline">Terms</a> and <a href="/privacy" className="underline">Privacy Policy</a>.</p>
//       </div>
//     </div>
//   );
// }
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // keep same behavior but store under lokniti_user for consistency
      localStorage.setItem("lokniti_user", JSON.stringify(res.data));
      toast.info("OTP sent to your email", { position: "top-center", autoClose: 2000 });
        ...
        <div className="bg-gradient-to-br from-[#041024] to-[#001429] rounded-2xl shadow-2xl p-8 border border-blue-800/20">
         ...
         <div className="flex items-center gap-3 mb-6">
          ...
          <form onSubmit={handleRegister} className="space-y-4" noValidate>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => setPassword(e.target.value)}
                required
                className="w-full rounded-lg bg-black/60 border border-blue-800/40 text-white p-3 pr-12 placeholder-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <button
                type="button"
                onClick={() => setShowPassword((s) => !s)}
          ...
          <p className="text-center text-sm text-blue-200 mt-4">
            ...
            <span onClick={() => navigate("/login")} className="text-white font-semibold hover:underline cursor-pointer">Sign in</span>
          </p>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3">
            <button
              type="button"
              onClick={() => toast.info("Google OAuth not set up yet")}
          ...
          <p className="mt-6 text-center text-xs text-blue-400">By creating an account, you agree to Lokniti AI's <Link to="/terms" className="underline">Terms</Link> and <Link to="/privacy" className="underline">Privacy Policy</Link>.</p>
        </div>
      </div>
    </div>
  );
